...
public class ControladorMensaje {
	...
	@PostMapping("/Enviausuario2")
	public ResponseEntity<Mensaje> enviarmensaje(@RequestBody Mensaje m){
		try {
			Queue cola = p.getConnectionFactory().createConnection().createSession().createQueue("Usuario2");
			
			Mensaje ob = new Mensaje(m.getComentario(), m.getTipo() + "[USUARIO2]", m.getFechaHoraTexto());
